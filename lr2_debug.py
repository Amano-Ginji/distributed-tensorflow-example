...
class LoadDataThread(threading.Thread):
    ...
    def run(self):
        for file_name in self.files:
            ...
            for line in input_file.readline(): # generator
                print "hello"
                #sample = Sample()
...
class DataProvider:
    def __init__(self, num_workers, task_index, mode='all'):
        ...
        self.pidx = 0
        self.thread_num = 2
        self.threads = self.InitThreads(self.file_list, len(self.file_list), self.thread_num)
        #self.procs = self.InitProcs(self.file_list, len(self.file_list), cpu_count()) # NOTE(yaowq): tf.gfile.GFile can't running in multiprocessing
